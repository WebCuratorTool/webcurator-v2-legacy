...
create table db_wct.TARGET (T_AT_OID number(19,0) not null, T_RUN_ON_APPROVAL number(1,0), T_EVALUATION_NOTE varchar2(1000), T_SELECTION_DATE TIMESTAMP(9), T_SELECTION_NOTE varchar2(1000), T_SELECTION_TYPE varchar2(255), T_HARVEST_TYPE varchar2(255), T_USE_AQA number(1,0) default 0 not null, primary key (T_AT_OID));
...
create table db_wct.TARGET_INSTANCE (TI_OID number(19,0) not null, TI_VERSION number(10,0) not null, TI_SCHEDULE_ID number(19,0), TI_TARGET_ID number(19,0), TI_PRIORITY number(10,0) not null, TI_SCHEDULED_TIME TIMESTAMP(9) not null, TI_STATE varchar2(50) not null, TI_BANDWIDTH_PERCENT number(10,0), TI_ALLOCATED_BANDWIDTH number(19,0), TI_START_TIME date, TI_OWNER_ID number(19,0), TI_DISPLAY_ORDER number(10,0), TI_PROF_OVERRIDE_OID number(19,0), TI_PURGED number(1,0) not null, TI_ARCHIVE_ID varchar2(40) unique, TI_REFERENCE varchar2(255), TI_HARVEST_SERVER varchar2(255), TI_DISPLAY_TARGET_INSTANCE number(1,0) default 1 not null, TI_DISPLAY_NOTE varchar2(4000), TI_FLAGGED number(1,0) default 0 not null, TI_PROFILE_ID number(19,0), TI_ARCHIVED_TIME date, TI_FIRST_FROM_TARGET number(1,0) default 0 not null, TI_DISPLAY_CHG_REASON varchar2(1000), TI_USE_AQA number(1,0) default 0 not null, primary key (TI_OID));
