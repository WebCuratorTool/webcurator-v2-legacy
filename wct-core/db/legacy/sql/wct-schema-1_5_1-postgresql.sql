...
create table DB_WCT.TARGET (T_AT_OID int8 not null, T_RUN_ON_APPROVAL bool, T_EVALUATION_NOTE varchar(1000), T_SELECTION_DATE TIMESTAMP(9), T_SELECTION_NOTE varchar(1000), T_SELECTION_TYPE varchar(255), T_HARVEST_TYPE varchar(255), primary key (T_AT_OID));
...
create table DB_WCT.TARGET_INSTANCE (TI_OID int8 not null, TI_VERSION int4 not null, TI_SCHEDULE_ID int8, TI_TARGET_ID int8, TI_PRIORITY int4 not null, TI_SCHEDULED_TIME TIMESTAMP(9) not null, TI_STATE varchar(50) not null, TI_BANDWIDTH_PERCENT int4, TI_ALLOCATED_BANDWIDTH int8, TI_START_TIME timestamp, TI_OWNER_ID int8, TI_DISPLAY_ORDER int4, TI_PROF_OVERRIDE_OID int8, TI_PURGED bool not null, TI_ARCHIVE_ID varchar(40) unique, TI_REFERENCE varchar(255), TI_HARVEST_SERVER varchar(255), TI_DISPLAY_TARGET_INSTANCE bool default true not null, TI_DISPLAY_NOTE varchar(4000), TI_FLAGGED bool default false not null, TI_PROFILE_ID int8, TI_ARCHIVED_TIME timestamp, TI_FIRST_FROM_TARGET boolean not null default false, TI_DISPLAY_CHG_REASON varchar(1000), primary key (TI_OID));
