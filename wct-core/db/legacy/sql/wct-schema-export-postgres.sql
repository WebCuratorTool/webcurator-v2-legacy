...
drop table DB_WCT.
            ID_GENERATOR
        ;
create table DB_WCT.ABSTRACT_TARGET (AT_OID int8 not null, AT_DESC varchar(255), AT_NAME varchar(255) unique, AT_OWNER_ID int8, AT_PROF_OVERRIDE_OID int8, AT_STATE int4, T_PROFILE_ID int8, AT_OBJECT_TYPE int4, AT_CREATION_DATE TIMESTAMP(9), AT_REFERENCE varchar(255), AT_PROFILE_NOTE varchar(255), AT_DUBLIN_CORE_OID int8, primary key (AT_OID));
...
create table DB_WCT.HARVEST_RESOURCE (HRC_OID int8 not null, HRC_LENGTH int8, HRC_NAME varchar(1020) not null, HRC_HARVEST_RESULT_OID int8, primary key (HRC_OID));
create table DB_WCT.HARVEST_RESULT (HR_OID int8 not null, HR_HARVEST_NO int4, HR_TARGET_INSTANCE_ID int8, HR_PROVENANCE_NOTE varchar(1024) not null, HR_CREATED_DATE timestamp, HR_CREATED_BY_ID int8, HR_STATE int4, HR_INDEX int4, primary key (HR_OID));
...
create table DB_WCT.PERMISSION_URLPATTERN (PU_URLPATTERN_ID int8 not null, PU_PERMISSION_ID int8 not null, primary key (PU_URLPATTERN_ID, PU_PERMISSION_ID));
...
create table DB_WCT.PROFILE (P_OID int8 not null, P_VERSION int4 not null, P_DESC varchar(255), P_NAME varchar(255), P_PROFILE_STRING text, P_PROFILE_LEVEL int4, P_STATUS int4, P_DEFAULT bool, P_AGECNY_OID int8, primary key (P_OID));
...
create table DB_WCT.SITE (ST_OID int8 not null, ST_TITLE varchar(255) not null unique, ST_DESC varchar(255), ST_LIBRARY_ORDER_NO varchar(32), ST_NOTES text, ST_PUBLISHED bool not null, ST_ACTIVE bool not null, primary key (ST_OID));
...
create table DB_WCT.TARGET_INSTANCE (TI_OID int8 not null, TI_VERSION int4 not null, TI_SCHEDULE_ID int8, TI_TARGET_ID int8, TI_PRIORITY int4 not null, TI_SCHEDULED_TIME TIMESTAMP(9) not null, TI_STATE varchar(50) not null, TI_BANDWIDTH_PERCENT int4, TI_ALLOCATED_BANDWIDTH int8, TI_START_TIME timestamp, TI_OWNER_ID int8, TI_DISPLAY_ORDER int4, TI_PROF_OVERRIDE_OID int8, TI_PURGED bool not null, TI_ARCHIVE_ID varchar(40) unique, TI_REFERENCE varchar(255), primary key (TI_OID));
...
create table DB_WCT.USER_ROLE (URO_USR_OID int8 not null, URO_ROL_OID int8 not null, primary key (URO_USR_OID, URO_ROL_OID));
...
create table DB_WCT.WCTUSER (USR_OID int8 not null, USR_ACTIVE bool not null, USR_ADDRESS varchar(200), USR_EMAIL varchar(100) not null, USR_EXTERNAL_AUTH bool not null, USR_FIRSTNAME varchar(50) not null, USR_FORCE_PWD_CHANGE bool not null, USR_LASTNAME varchar(50) not null, USR_NOTIFY_BY_EMAIL bool not null, USR_PASSWORD varchar(255), USR_PHONE varchar(16), USR_TITLE varchar(10), USR_USERNAME varchar(80) not null unique, USR_AGC_OID int8 not null, USR_DEACTIVATE_DATE TIMESTAMP(9), primary key (USR_OID));
