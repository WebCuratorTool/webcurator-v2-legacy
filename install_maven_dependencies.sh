...
## Install the specific fastutil version used by heritrix-1-14-adjust
mvn install:install-file -DgroupId=fastutil -DartifactId=fastutil \
      -Dversion=5.0.3-heritrix-subset-1.0 \
      -Dpackaging=jar \
      -Dfile=./wct-core/target/heritrix-1-14-adjust/lib/fastutil-5.0.3-heritrix-subset-1.0.jar
## Install the specific mg4j version used by heritrix-1-14-adjust
mvn install:install-file -DgroupId=it.unimi.dsi -DartifactId=mg4j \
      -Dversion=1.0.1 \
      -Dpackaging=jar \
      -Dfile=./wct-core/target/heritrix-1-14-adjust/lib/mg4j-1.0.1.jar
## Install the javaswf dependency used by heritrix-1-14-adjust
