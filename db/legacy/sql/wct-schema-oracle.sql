...
drop table DB_WCT.ABSTRACT_TARGET_GROUPTYPE_VIEW cascade constraints;
drop table DB_WCT.ABSTRACT_TARGET_SCHEDULE_VIEW cascade constraints;
...
drop table DB_WCT.URL_PERMISSION_MAPPING_VIEW cascade constraints;
...
create table DB_WCT.AGENCY (AGC_OID number(19,0) not null, AGC_NAME varchar2(80) not null unique, AGC_ADDRESS varchar2(255) not null, AGC_LOGO_URL varchar2(255), AGC_URL varchar2(255), AGC_EMAIL varchar2(80), AGC_FAX varchar2(20), AGC_PHONE varchar2(20), AGC_SHOW_TASKS number(1,0), AGC_DEFAULT_DESC_TYPE varchar2(255), primary key (AGC_OID));
create table DB_WCT.ANNOTATIONS (AN_OID number(19,0) not null, AN_DATE date not null, AN_NOTE varchar2(1000) not null, AN_USER_OID number(19,0) not null, AN_OBJ_OID number(19,0) not null, AN_OBJ_TYPE varchar2(500) not null, AN_ALERTABLE number(1,0), primary key (AN_OID));
...
create table DB_WCT.BANDWIDTH_RESTRICTIONS (BR_OID number(19,0) not null, BR_BANDWIDTH number(19,0) not null, BR_DAY varchar2(9) not null, BR_END_TIME date not null, BR_START_TIME date not null, BR_OPTIMIZATION_ALLOWED number(1,0), primary key (BR_OID));
...
create table DB_WCT.FLAG (F_OID number(19,0) not null, F_NAME varchar2(255) not null, F_RGB varchar2(255) not null, F_COMPLEMENT_RGB varchar2(255) not null, F_AGC_OID number(19,0) not null, primary key (F_OID));
...
create table DB_WCT.HARVEST_RESULT (HR_OID number(19,0) not null, HR_HARVEST_NO number(10,0), HR_TARGET_INSTANCE_ID number(19,0), HR_PROVENANCE_NOTE varchar2(1024) not null, HR_CREATED_DATE date, HR_CREATED_BY_ID number(19,0), HR_RR_OID number(19,0), HR_STATE number(10,0), HR_DERIVED_FROM number(10,0), HR_INDEX number(10,0), primary key (HR_OID));
...
create table DB_WCT.HEATMAP_CONFIG (HM_OID number(19,0) not null, HM_NAME varchar2(255) not null, HM_COLOR varchar2(255) not null, HM_THRESHOLD_LOWEST number(10,0) not null, HM_DISPLAY_NAME varchar2(255) not null, primary key (HM_OID));
...
create table DB_WCT.INDICATOR (I_OID number(19,0) not null, I_IC_OID number(19,0) not null, I_NAME varchar2(255) not null, I_TI_OID number(19,0), I_FLOAT_VALUE float not null, I_UPPER_LIMIT_PERCENTAGE float, I_LOWER_LIMIT_PERCENTAGE float, I_UPPER_LIMIT float, I_LOWER_LIMIT float, I_ADVICE varchar2(255), I_JUSTIFICATION varchar2(255), I_AGC_OID number(19,0) not null, I_UNIT varchar2(255) not null, I_SHOW_DELTA number(1,0) not null, I_DATE TIMESTAMP(9) not null, I_INDEX number(10,0), primary key (I_OID));
create table DB_WCT.INDICATOR_CRITERIA (IC_OID number(19,0) not null, IC_NAME varchar2(255) not null, IC_DESCRIPTION varchar2(255), IC_UPPER_LIMIT_PERCENTAGE float, IC_LOWER_LIMIT_PERCENTAGE float, IC_UPPER_LIMIT float, IC_LOWER_LIMIT float, IC_AGC_OID number(19,0) not null, IC_UNIT varchar2(255) not null, IC_SHOW_DELTA number(1,0) not null, IC_ENABLE_REPORT number(1,0) not null, primary key (IC_OID));
create table DB_WCT.INDICATOR_REPORT_LINE (IRL_OID number(19,0) not null, IRL_I_OID number(19,0), IRL_LINE varchar2(255), IRL_INDEX number(10,0), primary key (IRL_OID));
...
create table DB_WCT.PERMISSION (PE_OID number(19,0) not null, PE_ACCESS_STATUS varchar2(255), PE_APPROVED_YN number(1,0), PE_AVAILABLE_YN number(1,0), PE_COPYRIGHT_STATEMENT varchar2(2048), PE_COPYRIGHT_URL varchar2(2048), PE_CREATION_DATE TIMESTAMP(9) NULL, PE_END_DATE TIMESTAMP(9) NULL, PE_NOTES clob, PE_OPEN_ACCESS_DATE TIMESTAMP(9) NULL, PE_PERMISSION_GRANTED_DATE TIMESTAMP(9) NULL, PE_PERMISSION_REQUESTED_DATE TIMESTAMP(9) NULL, PE_SPECIAL_REQUIREMENTS varchar2(2048), PE_START_DATE TIMESTAMP(9) NULL, PE_STATUS number(10,0), PE_AUTH_AGENT_ID number(19,0), PE_SITE_ID number(19,0), PE_QUICK_PICK number(1,0), PE_DISPLAY_NAME varchar2(32), PE_OWNING_AGENCY_ID number(19,0), PE_FILE_REFERENCE varchar2(255), primary key (PE_OID));
...
create table DB_WCT.PERMISSION_TEMPLATE (PRT_OID number(19,0) not null, PRT_AGC_OID number(19,0) not null, PRT_TEMPLATE_TEXT clob not null, PRT_TEMPLATE_NAME varchar2(80) not null, PRT_TEMPLATE_TYPE varchar2(40) not null, PRT_TEMPLATE_DESC varchar2(255), PRT_TEMPLATE_SUBJECT varchar2(255), PRT_TEMPLATE_OVERWRITE_FROM number(1,0) not null, PRT_TEMPLATE_FROM varchar2(255), PRT_TEMPLATE_CC varchar2(2048), PRT_TEMPLATE_BCC varchar2(2048), PRT_TEMPLATE_REPLY_TO varchar2(255), primary key (PRT_OID));
...
create table DB_WCT.REJECTION_REASON (RR_OID number(19,0) not null, RR_NAME varchar2(100) not null, RR_AGC_OID number(19,0) not null, RR_AVAILABLE_FOR_TARGET number(1,0), RR_AVAILABLE_FOR_TI number(1,0), primary key (RR_OID));
...
create table DB_WCT.SCHEDULE (S_OID number(19,0) not null, S_CRON varchar2(255) not null, S_START TIMESTAMP(9) not null, S_END TIMESTAMP(9) NULL, S_TARGET_ID number(19,0), S_TYPE number(10,0) not null, S_OWNER_OID number(19,0), S_NEXT_SCHEDULE_TIME TIMESTAMP(9) NULL, S_LAST_PROCESSED_DATE TIMESTAMP(9) NULL, S_ABSTRACT_TARGET_ID number(19,0), primary key (S_OID));
...
create table DB_WCT.SEED_HISTORY (SH_OID number(19,0) not null, SH_SEED varchar2(1024), SH_TI_OID number(19,0), SH_PRIMARY number(1,0), primary key (SH_OID));
...
create table DB_WCT.SITE (ST_OID number(19,0) not null, ST_TITLE varchar2(255) not null unique, ST_DESC varchar2(4000), ST_LIBRARY_ORDER_NO varchar2(32), ST_NOTES clob, ST_PUBLISHED number(1,0) not null, ST_CREATION_DATE TIMESTAMP(9) NULL, ST_ACTIVE number(1,0) not null, ST_OWNING_AGENCY_ID number(19,0), primary key (ST_OID));
...
create table DB_WCT.TARGET (T_AT_OID number(19,0) not null, T_RUN_ON_APPROVAL number(1,0), T_USE_AQA number(1,0), T_EVALUATION_NOTE varchar2(255), T_SELECTION_DATE TIMESTAMP(9) NULL, T_SELECTION_NOTE varchar2(255), T_SELECTION_TYPE varchar2(255), T_HARVEST_TYPE varchar2(255), T_ALLOW_OPTIMIZE number(1,0), primary key (T_AT_OID));
...
create table DB_WCT.TARGET_INSTANCE (TI_OID number(19,0) not null, TI_VERSION number(10,0) not null, TI_SCHEDULE_ID number(19,0), TI_TARGET_ID number(19,0), TI_PRIORITY number(10,0) not null, TI_SCHEDULED_TIME TIMESTAMP(9) not null, TI_STATE varchar2(50) not null, TI_BANDWIDTH_PERCENT number(10,0), TI_ALLOCATED_BANDWIDTH number(19,0), TI_START_TIME date, TI_ARCHIVED_TIME date, TI_OWNER_ID number(19,0), TI_DISPLAY_ORDER number(10,0), TI_PROF_OVERRIDE_OID number(19,0), TI_PROFILE_ID number(19,0), TI_PURGED number(1,0) not null, TI_ARCHIVE_ID varchar2(40) unique, TI_REFERENCE varchar2(255), TI_HARVEST_SERVER varchar2(255), TI_DISPLAY_TARGET_INSTANCE number(1,0), TI_DISPLAY_NOTE varchar2(4000), TI_DISPLAY_CHG_REASON varchar2(1000), TI_FLAGGED number(1,0), TI_FLAG_OID number(19,0), TI_USE_AQA number(1,0), TI_FIRST_FROM_TARGET number(1,0), TI_RECOMMENDATION varchar2(255), TI_ALLOW_OPTIMIZE number(1,0), primary key (TI_OID));
...
create table DB_WCT.URL_PERMISSION_MAPPING_VIEW (UPM_OID number(19,0) not null, PE_END_DATE TIMESTAMP(9) NULL, UP_PATTERN varchar2(2048), PE_OWNING_AGENCY_ID number(19,0), PE_OID number(19,0), ST_ACTIVE number(1,0) not null, UPM_DOMAIN varchar2(1024), primary key (UPM_OID));
