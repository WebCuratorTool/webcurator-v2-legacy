...
create table DB_WCT.ABSTRACT_TARGET (AT_OID number(19,0) not null, AT_DESC varchar2(4000), AT_NAME varchar2(255) unique, AT_OWNER_ID number(19,0), AT_PROF_OVERRIDE_OID number(19,0), AT_STATE number(10,0), T_PROFILE_ID number(19,0), AT_OBJECT_TYPE number(10,0), AT_CREATION_DATE TIMESTAMP(9), AT_REFERENCE varchar2(255), AT_PROFILE_NOTE varchar2(255), AT_DUBLIN_CORE_OID number(19,0), AT_ACCESS_ZONE number(2,0) default 0 not null, AT_DISPLAY_TARGET number(1,0) default 1 not null, AT_DISPLAY_NOTE varchar2(4000), primary key (AT_OID));
...
create table DB_WCT.ANNOTATIONS (AN_OID number(19,0) not null, AN_DATE date not null, AN_NOTE varchar2(1000) not null, AN_USER_OID number(19,0) not null, AN_OBJ_OID number(19,0) not null, AN_OBJ_TYPE varchar2(500) not null, primary key (AN_OID));
...
create table DB_WCT.HARVEST_STATUS (HS_OID number(19,0) not null, HS_AVG_KB double precision, HS_AVG_URI double precision, HS_DATA_AMOUNT number(19,0), HS_ELAPSED_TIME number(19,0), HS_JOB_NAME varchar2(500), HS_STATUS varchar2(255), HS_URLS_DOWN number(19,0), HS_URLS_FAILED number(19,0), HS_ALERTS number(10,0), primary key (HS_OID));
...
create table DB_WCT.PERMISSION_TEMPLATE (PRT_OID number(19,0) not null, PRT_AGC_OID number(19,0) not null, PRT_TEMPLATE_TEXT clob not null, PRT_TEMPLATE_NAME varchar2(80) not null, PRT_TEMPLATE_TYPE varchar2(40) not null, PRT_TEMPLATE_DESC varchar2(255), primary key (PRT_OID));
...
create table DB_WCT.PROFILE (P_OID number(19,0) not null, P_VERSION number(10,0) not null, P_DESC varchar2(255), P_NAME varchar2(255), P_PROFILE_STRING clob, P_PROFILE_LEVEL number(10,0), P_STATUS number(10,0), P_DEFAULT number(1,0), P_AGECNY_OID number(19,0), primary key (P_OID));
...
create table DB_WCT.SCHEDULE (S_OID number(19,0) not null, S_CRON varchar2(255) not null, S_START TIMESTAMP(9) not null, S_END TIMESTAMP(9), S_TARGET_ID number(19,0), S_TYPE number(10,0) not null, S_OWNER_OID number(19,0), S_NEXT_SCHEDULE_TIME TIMESTAMP(9), S_ABSTRACT_TARGET_ID number(19,0), primary key (S_OID));
...
create table DB_WCT.SITE (ST_OID number(19,0) not null, ST_TITLE varchar2(255) not null unique, ST_DESC varchar2(4000), ST_LIBRARY_ORDER_NO varchar2(32), ST_NOTES clob, ST_PUBLISHED number(1,0) not null, ST_ACTIVE number(1,0) not null, primary key (ST_OID));
...
create table DB_WCT.TARGET (T_AT_OID number(19,0) not null, T_RUN_ON_APPROVAL number(1,0), T_EVALUATION_NOTE varchar2(255), T_SELECTION_DATE TIMESTAMP(9), T_SELECTION_NOTE varchar2(255), T_SELECTION_TYPE varchar2(255), T_HARVEST_TYPE varchar2(255), primary key (T_AT_OID));
...
create table DB_WCT.TARGET_INSTANCE (TI_OID number(19,0) not null, TI_VERSION number(10,0) not null, TI_SCHEDULE_ID number(19,0), TI_TARGET_ID number(19,0), TI_PRIORITY number(10,0) not null, TI_SCHEDULED_TIME TIMESTAMP(9) not null, TI_STATE varchar2(50) not null, TI_BANDWIDTH_PERCENT number(10,0), TI_ALLOCATED_BANDWIDTH number(19,0), TI_START_TIME date, TI_OWNER_ID number(19,0), TI_DISPLAY_ORDER number(10,0), TI_PROF_OVERRIDE_OID number(19,0), TI_PURGED number(1,0) not null, TI_ARCHIVE_ID varchar2(40) unique, TI_REFERENCE varchar2(255), TI_HARVEST_SERVER varchar2(255), TI_DISPLAY_TARGET_INSTANCE number(1,0) default 1 not null, TI_DISPLAY_NOTE varchar2(4000), primary key (TI_OID));
