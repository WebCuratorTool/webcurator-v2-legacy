...
public class ArcDigitalAssetStoreService implements DigitalAssetStore,
		...
		try {
			...
			for (int i = 0; i < arcFiles.length; i++) {
				...
				if (reader instanceof ARCReader) {
					...
					ARCWriter writer = new ARCWriter(aint, dirs, prefix,
							suffix, compressed,
							ARCReader.DEFAULT_MAX_ARC_FILE_SIZE, l);
					...
					WARCWriter writer = new WARCWriter(aint, dirs, prefix,
							suffix, compressed,
							WARCReader.DEFAULT_MAX_WARC_FILE_SIZE, l);
			...
			if (!hrsToImport.isEmpty()) {
				...
				if (impArcType.equals("ARC")) {
					// Create an ARC Writer
					ARCWriter arcWriter = new ARCWriter(aint, dirs,
							impArcPrefix, impArcSuffix, compressit,
							ARCReader.DEFAULT_MAX_ARC_FILE_SIZE, impArcHeader);
					...
					// Create a WARC Writer
					WARCWriter warcWriter = new WARCWriter(aint, dirs,
							impArcPrefix, impArcSuffix, compressit,
							WARCReader.DEFAULT_MAX_WARC_FILE_SIZE, impArcHeader);
					for (Iterator<HarvestResourceDTO> it = hrsToImport
							.iterator(); it.hasNext();) {
						HarvestResourceDTO hr = it.next();
						if (hr.getLength() > 0L) {
	 ...
	 * 
	 * @param hrs
	 *            - A list of HarvestResourceDTO objects
	 * @param Url
	...
	public void purge(String[] targetInstanceNames)
		...
		try {
			for (String tiName : targetInstanceNames) {
				...
				if(!FileUtils.deleteDir(toPurge)) {
					if(log.isWarnEnabled()) {
